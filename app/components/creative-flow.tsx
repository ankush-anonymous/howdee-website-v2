...
export default function PersonalizedCreativeFlow() {
  ...
  return (
    <div className="flex w-full min-h-screen font-sans" style={{ backgroundColor: "#fbf5df" }}>
      {/* Left Sticky Side */}
      <div className="w-1/2 sticky top-0 h-screen flex items-center justify-center relative overflow-hidden">
        <div className="absolute top-10 left-10 w-16 h-16 rounded-full opacity-10 animate-pulse" style={{ backgroundColor: "#ff6b6b" }}></div>
        <div className="absolute bottom-20 right-10 w-12 h-12 rounded-full opacity-15 animate-pulse" style={{ backgroundColor: "#ff6b6b", animationDelay: "1s" }}></div>
        <div className="absolute top-1/3 right-20 w-8 h-8 rounded-full opacity-20 animate-pulse" style={{ backgroundColor: "#ff6b6b", animationDelay: "2s" }}></div>
        ...
        <div className="text-center space-y-6 z-10 px-8">
          ...
          <div className="flex justify-center gap-4 py-6">
            {steps.map((_, index) => (
              <motion.div
                key={index}
                className={`w-4 h-4 rounded-full transition-all duration-300 ${index === currentStep ? "scale-125 shadow-lg" : "bg-gray-300"}`}
                style={{ backgroundColor: index === currentStep ? "#ff6b6b" : "#d1d5db" }}
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              style={{ backgroundColor: "#fbf5df", borderColor: "rgba(255, 107, 107, 0.2)" }}
            >
              ...
              <p className="text-gray-700 text-lg font-medium">{steps[currentStep].description}</p>
            </motion.div>
          </AnimatePresence>
        </div>
      </div>
      ...
      <div className="w-1/2 flex flex-col">
        {steps.map((step, index) => (
          <div
            ...
            style={{ background: `linear-gradient(135deg, #fbf5df 0%, rgba(255, 107, 107, 0.05) 100%)` }}
          >
            <motion.div
            ...
            >
              ...
              <p className="text-gray-600 text-lg mb-6 font-medium">{step.description}</p>
              <motion.div
              ...
              >
                ...
                <p className="text-sm font-medium text-center px-4">{step.placeholder}</p>
              </motion.div>
            </motion.div>
          </div>
        ))}
